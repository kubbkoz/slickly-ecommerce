...
extends: ["../vue-starter-template", "./features/blog"],
  ...
  image: {
    // REVERTED: forcing the built-in IPX provider made Nitro fetch each Shopware
    // original server-side — but that server-side fetch to admin.slickly.sk gets
    // blocked/degraded (no browser-like Referer/headers), which broke PDP images
    // entirely and produced low-quality re-encodes elsewhere. Back to the
    // inherited "shopware" provider from @shopware/cms-base-layer, which the
    // browser fetches directly (known-working state).
