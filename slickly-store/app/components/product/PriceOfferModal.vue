...
import { Zap, Loader2, Check } from 'lucide-vue-next';
...
const handleSubmit = async () => {
  ...
  state.value = 'loading';
  try {
    const res = await $fetch<any>('/api/price-offer/submit', {
      method: 'POST',
      body: {
        ...
        competitorUrl: competitorUrl.value.trim() || undefined,
...
<template>
  <AppModal :is-open="isOpen" title="Dopyt na cenovú ponuku" @close="emit('close')">
    <div class="px-2 py-4">
      <!-- Success -->
      <div v-if="state === 'success'" class="text-center py-8 animate-fade-in">
        ...
        <h3 class="font-tech font-bold uppercase text-lg mb-2">Dopyt odoslaný</h3>
        <p class="text-gray-500 text-sm font-sans mb-6">Ďakujeme za Váš záujem. Ozveme sa Vám čo najskôr s cenovou ponukou.</p>
      ...
      <!-- Form -->
      <template v-else>
        <p class="text-sm text-gray-500 font-sans mb-6">Zanechajte nám kontakt a pripravíme Vám cenovú ponuku pre <strong class="text-black">{{ product?.name }}</strong>.</p>
        ...
        <div class="space-y-4">
          ...
          <div>
            <label class="form-label">Našli ste lepšiu cenu? (voliteľné)</label>
            <input v-model="competitorUrl" type="url" class="form-input w-full" placeholder="https://..." />
          </div>
          ...
          <label class="flex items-start gap-2 cursor-pointer">
            ...
            <span class="text-xs text-gray-500 font-sans">Súhlasím so spracovaním osobných údajov za účelom spracovania cenovej ponuky. *</span>
          </label>
        </div>
        ...
        >
          ...
          {{ state === 'loading' ? 'Odosielam...' : 'Odoslať dopyt' }}
        </button>
      </template>
    </div>
  </AppModal>
